	    ...
	    data:
	    	{
	       	...
	       	success: function( data ) {
		   		    ...
		   		    window.location=data;	//changed
		    },
		   
		});
}
/*Function to add the site admin of an organization */
 ...
 /*close user profile popup*/
   ...
   function viewDivision(divisionId){
	   ...
	   document.getElementById('division_profile_popup').style.display ="inline-block";
		 var $body=$('body');
		 $('#form_divisionName').val($('#divisionName'+divisionId).text().trim());
		 $body.addClass('loading');
		 var divisionName1=$("#division_profile_popup").fadeIn(1000);
		 positionPopup();
		//var divisionName1=$('#divisionName').val();
		alert(divisionName1);
		$("#focus_div").attr("tabindex",1).focus(); 
		 $.ajax({
			    url: "/ezXpense/siteadmin/editprofile.do",
			    type: 'GET',
			    data:
			    	{
			    ...
			    success: function( data ) {
				   		    alert("Division edited Successfully ");
				   		    window.location="/ezXpense/siteadmin/division.do";	//changed			       	
				    },
...
<body class="full-layout  nav-right-hide nav-right-start-hide  nav-top-fixed responsive clearfix"data-active="dashboard " data-smooth-scrolling="1">
	<div class="vd_body">
		                      	 ...
		                      	 <td>
						    		...
						    		<button class="org_submit" type="submit"  onclick="addDivision()">Save</button>
											...
											<c:forEach items="${divisionsDtos}" var="divisionsDto">
							   					<tr>	
								   					<td id="divisionName${divisionsDto.divisionId}">${divisionsDto.divisionName}</td>
													<td><form action=""></form>
													<a class="btn btn-success btn-xs has-tooltip" href="#"  id="division_profile"  onclick="viewDivision(${divisionsDto.divisionId});">
	  ...
	  <div id="division_profile_popup" class="division_profile_popup" style="left: 350px; top: 200px; position: absolute;display:none;">
	         <a href="#" id="close_popup" onclick="closeUserProfilePopup();"><img src="${pageContext.request.contextPath}/images/close_icon1.png"  height="28px;" width="28px;" style="float:right;"/></a> 
	         <br/>
	         <div id="header1"> EDIT DIVISION NAME  </div>
	         <form:form class="divisionProfileForm" id="divisionProfileForm" action="${pageContext.request.contextPath}/siteadmin/editprofile.do" commandName="divisionsDto" method="GET" >
